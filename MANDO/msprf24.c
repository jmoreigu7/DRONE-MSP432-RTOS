...
void msprf24_init()
{
	...
	//-- setup IRQ for pins avaible on NRF24L01 boosterpack
	//-- GPIOs avaible at boosterpack from left to right counter clockwise
	//-- P4.6
	//-- P4.1
	//-- P2.3
	//-- P5.1
	...
	#if nrfIRQport == 6
        P6->SEL1 &=~nrfIRQpin;  //-- configure P3.5 as simple I/O
		P6->SEL0 &=~nrfIRQpin;
		P6->DIR &= ~nrfIRQpin;  //-- IRQ line is input
		P6->OUT |= nrfIRQpin;   //-- pull-up resistor enabled
		P6->REN |= nrfIRQpin;
		P6->IES |= nrfIRQpin;   //-- trigger on falling-edge
		P6->IFG &= ~nrfIRQpin;  //-- clear any outstanding IRQ
		P6->IE |= nrfIRQpin;    //-- enable IRQ interrupt from P3.5
	#endif

	//-- arm microcontroller needs enable interrupts from NVIC registers

	NVIC_SetPriority(PORT6_IRQn,3); //-- set priority to 3 in NVIC
	NVIC_EnableIRQ(PORT6_IRQn); //-- enable interrupt IRQs
	__enable_irq();
